'''
    ...
    Given an array of intervals [startTime, endTime], merge all overlapping intervals and 
...
def mergeHighDefinitionIntervals(intervals):
    ...
    # Step 1: Ordina per start time
    intervals.sort(key=lambda x: x[0])
    
    # Step 2: Inizia con il primo intervallo
    result = [intervals[0]]
    
    # Step 3: Itera attraverso gli altri intervalli
    for i in range(1, len(intervals)):
        current = intervals[i]
        last_merged = result[-1]
        
        # Controlla se c'è overlap
        if current[0] <= last_merged[1]:
            # C'è overlap - mergia gli intervalli
            last_merged[1] = max(last_merged[1], current[1])
        else:
            # Nessun overlap - aggiungi nuovo intervallo
