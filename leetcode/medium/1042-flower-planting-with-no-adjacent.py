'''
    ...
    of flower planted in the (i+1)th garden. 
...
def gardenNoAdj(n: int, paths: List[List[int]]) -> List[int]:
    ...
    # result array (0 = not yet colored)
    result = [0] * (n + 1) # 0-index not used

    # for each garden
    for garden in range(1, n + 1):
        # find already used colors
        used_colors = set()
        for neighbour in graph[garden]:
            if result[neighbour] != 0:
                used_colors.add(result[neighbour])
        # find first avaiable color between {1, 2, 3, 4}
